...
export function CellWithTooltip({
  ...
  return (
    <td className={`px-2 py-1 text-right text-xs tabular-nums ${color}`}>
      <HoverCard openDelay={200} closeDelay={100}>
        <HoverCardTrigger asChild>
          <span className={`cursor-help border-b border-dotted border-foreground/40 ${dimmed ? 'opacity-40' : ''}`}>{fmt(value)}</span>
        </HoverCardTrigger>
